...
SubWindow::SubWindow(ifstream &fin)
{
	...
	string dataForLists;
	getline(fin, dataForLists);
	while (dataForLists == "")
		getline(fin, dataForLists);

	ifstream fffin(dataForLists);
	int numberOfLists;
	fffin >> numberOfLists;
	shared_ptr<List> tempList;
	for (int i = 0; i < numberOfLists; ++i)
	{
		tempList = shared_ptr<List>(new List(fffin));
		allLists.push_back(tempList);
	}
	fffin.close();
...
vector<shared_ptr<List>> SubWindow::getAllLists()
{
	return allLists;
}
